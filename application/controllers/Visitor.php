...
class Visitor extends MY_Controller{
    ...
    */
    ...
    //-----------------
    //-----getVisiotrList
    function adminList($seg=1){
      $where = array(
        "v_ip !=" => $this->cur_ip
      );
      $get = $this->Mdl_visitor->adminList($where)->result();
      $num = count($get);
      //---pagination
      $url = "adminGetList";
      $per_page = 10;
      ...
      $get_v = $this->Mdl_visitor->adminList($where,$per_page,$start)->result();
      if($num >= $per_page):
